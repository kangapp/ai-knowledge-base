...
## Bug 43: 成功零结果未刷新数据源健康状态

**时间**：2026-06-20

**现象**：定向重跑 `rss_verge` 和 `rss_arstechnica` 均正常完成且返回 0 条，但健康页仍显示数日前的 `analysis_failed`。

**根因**：来源汇总仅从采集条目、错误和费用记录反推本轮来源。成功但无条目、无错误、无费用时不会写入 `pipeline_source_runs`，旧记录因此一直被视为最新状态。

**修复**：

- 将本轮 `active_sources` 显式传入来源汇总。
- 先为每个目标来源创建零值汇总，再叠加采集、分析、审核、错误和费用数据。
- 增加成功零结果仍写入来源汇总的回归测试。
