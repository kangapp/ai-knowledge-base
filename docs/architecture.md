...
- **GitHub 采集口径**：Search API 查询由最多 5 个 `topic:` qualifier / 显式 `keywords` include 条件组成，避免触发 GitHub `AND/OR/NOT` 操作符上限；`exclude_terms` 在返回 repo 后本地过滤；增速源基于 `github_repo_snapshots` 的最新快照与窗口前最近基线快照计算 star/day。
